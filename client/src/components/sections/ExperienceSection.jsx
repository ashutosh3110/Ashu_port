...
export default function ExperienceSection() {
  ...
  return (
    <section id="experience" className="py-24 relative z-10 bg-slate-100/50 dark:bg-slate-950/40">
            ...
            return (
              ...
              >
                ...
                <div className="hidden sm:block absolute -left-36 top-1.5 text-right w-24">
                  ...
                  <span className="text-[10px] text-slate-600 dark:text-slate-400 block font-semibold">{item.endDate}</span>
                </div>
                ...
                <GlassCard className="space-y-4">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs font-semibold text-slate-700 dark:text-slate-400 flex items-center space-x-2">
                        <span>{item.company}</span>
                        {item.location && (
                          <>
                            <span>•</span>
                            <span className="flex items-center gap-1">
                              <MapPin className="w-3 h-3 text-slate-500 dark:text-slate-500" />
                              {item.location}
                            </span>
                          </>
                        )}
                  ...
                  <p className="text-xs text-slate-800 dark:text-slate-300 leading-relaxed font-medium">{item.description}</p>

                  {item.highlights && item.highlights.length > 0 && (
                    <ul className="space-y-1.5 pt-2 border-t border-slate-200 dark:border-slate-800/80">
                      {item.highlights.map((h, i) => (
                        <li key={i} className="text-xs text-slate-700 dark:text-slate-400 font-medium flex items-start space-x-2">
